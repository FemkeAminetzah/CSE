...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, lr, cliprange,
        ...
        def get_loss(model, placeholder_dict):
            ...
            clipfrac = tf.reduce_mean(tf.cast(tf.greater(tf.abs(ratio - 1.0), clip_range), dtype=tf.float32))
        ...
        if log_interval > 0:
            ...
            tf.compat.v1.summary.histogram("others/neglocpac", neglogpac)
            tf.compat.v1.summary.histogram("others/vf", vpred)
        ...
        def train(obs, returns, masks, actions, values, neglogpacs, states=None):
            ...
            if log_interval > 0 and (self.num_steps_trained % self.log_interval == 0):
                ...
                self.summary_writer.add_summary(tf.compat.v1.Summary.FromString(summary_str), gs)
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, nenvs, envsperbatch):
                    ...
                    if nenvs == 1:
                        ...
                        if type(states) == tuple or type(states) == tf.nn.rnn_cell.LSTMStateTuple:  # LSTM state
