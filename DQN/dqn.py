...
class DQNAgent():
    ...
    def __init__(self, q_network, ob_space, ac_space,
        ...
        QT = tf.compat.v1.placeholder(shape=[batch_size*trace_length], dtype=tf.float32, name='QT')  # target Q values
        A = tf.compat.v1.placeholder(shape=[batch_size*trace_length], dtype=tf.int32, name='A')  # action indices
        ...
        td_error = tf.compat.v1.losses.mean_squared_error(labels=QT,
                                                predictions=tf.squeeze(tf.matmul(tf.multiply(train_model.predQ, actions_onehot), [[1.], [1.]])))
        loss = td_error
        params = tf.compat.v1.trainable_variables()  # was set to 'model', but we would need model and target parameters
        ...
        if log_interval > 0:
            for g, v in grads:
                if g is not None:
                    tf.compat.v1.summary.histogram("train/grads/%s-grad" % v.name.replace(':', '_'), g)
            for p in params:
                if p is not None:
                    tf.compat.v1.summary.histogram("train/params/%s" % p.name.replace(':', '_'), p.value())
            tf.compat.v1.summary.scalar("train/vf_loss", loss)
            tf.compat.v1.summary.histogram("others/A", A)
            tf.compat.v1.summary.histogram("others/QT", QT)
            self.summary_step = tf.compat.v1.summary.merge_all()

        tf.compat.v1.add_to_collection('inputs', eval_model.X)
        tf.compat.v1.add_to_collection('predQ', eval_model.predQ)
        ...
        def update_target(target_op_holder):
            ...
            b = tf.compat.v1.trainable_variables()[len(params)//2].eval(session=sess)
            if not a.all() == b.all():
