...
def restore_dqn_model(sess, logdir, f_name, isrnn):
    ...
    # restore the model
    loader = tf.compat.v1.train.import_meta_graph(glob.glob(os.path.join(logdir, (f_name + '.meta')))[0])  #TODO !!!
    ...
    obs_in = tf.compat.v1.get_collection('inputs')        # network inputs: observations
    predQ_out = tf.compat.v1.get_collection('predQ')      # get predicted Q values
    if isrnn:
