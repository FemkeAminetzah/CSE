...
class CategoricalPd:
    ...
    def neglogp(self, x):
        ...
        return tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=tf.stop_gradient(one_hot_actions))
...
def fc(x, scope, nh, *, init_scale=1.0, init_bias=0.0):
    with tf.compat.v1.variable_scope(scope):
        ...
        nin = x.shape[1]
        w = tf.compat.v1.get_variable("w", [nin, nh], initializer=tf.compat.v1.orthogonal_initializer(gain=init_scale))
...
def add_to_collection_rnn_state(name, rnn_state):
    ...
    for layer in rnn_state:
        ...
        tf.compat.v1.add_to_collection(coll_of_names, n)
        try:
            for l in layer:
                tf.compat.v1.add_to_collection(name, l)
        except TypeError:
            # layer is not iterable so just add it directly
            tf.compat.v1.add_to_collection(name, layer)
...
def get_collection_rnn_state(name):
    ...
    """
    layers = []
    coll = tf.compat.v1.get_collection(name)
    coll_of_names = tf.compat.v1.get_collection(name + '__names__')
    idx = 0
    for n in coll_of_names:
        if 'LSTMStateTuple' in n:
            state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(coll[idx], coll[idx+1])
            idx += 2
