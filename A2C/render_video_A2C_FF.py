...
SEED = 100
LOGDIR = 'C:/Users/mackj/OneDrive/Desktop'
...
with tf.compat.v1.Session() as sess:
    ...
    # restore the model
    loader = tf.compat.v1.train.import_meta_graph(glob.glob(os.path.join(LOGDIR, (F_NAME + '.meta')))[0])
    ...
    obs_in = tf.compat.v1.get_collection('inputs')
    probs_out = tf.compat.v1.get_collection('pi')
    pi_logits_out = tf.compat.v1.get_collection('pi_logit')
    predict_vf_op = tf.compat.v1.get_collection('val')
    predict_ac_op = tf.compat.v1.get_collection('step')
