...
class Model(object):
    ...
    def __init__(self, policy, ob_space, ac_space,
        ...
        self.global_step = tf.compat.v1.get_variable('global_step',
                                           [],
                                           tf.int32,
                                           tf.compat.v1.constant_initializer(0, tf.int32),
                                           trainable=False)

        eval_model = policy(sess, ob_space, ac_space, 1, 1, units_per_hlayer, reuse=False, activ_fcn=activ_fcn)
        step_model = policy(sess, ob_space, ac_space, nenvs, 1, units_per_hlayer, reuse=tf.compat.v1.AUTO_REUSE, activ_fcn=activ_fcn)
        ...
        if log_interval > 0:
            for g, v in gradients:
                if g is not None:
                    tf.compat.v1.summary.histogram("%s-grad" % v.name.replace(':', '_'), g)
            for p in params:
                if p is not None:
                    tf.compat.v1.summary.histogram("train/%s" % p.name.replace(':', '_'), p.value())
            tf.compat.v1.summary.scalar("train/pg_loss", pg_loss)
            tf.compat.v1.summary.scalar("train/vf_loss", vf_loss)
            tf.compat.v1.summary.scalar("train/entropy", entropy)
            tf.compat.v1.summary.histogram("others/ADV", ADV)
            tf.compat.v1.summary.histogram("others/neglocpac", chosen_action_log_probs)
            tf.compat.v1.summary.histogram("others/vf", vf)
            self.summary_step = tf.compat.v1.summary.merge_all()

        # Adding these to collection so we can restore them again
        tf.compat.v1.add_to_collection('inputs', eval_model.X)
        tf.compat.v1.add_to_collection('pi', eval_model.pi)
        tf.compat.v1.add_to_collection('pi_logit', eval_model.pi_logit)
        tf.compat.v1.add_to_collection('val', eval_model.vf)
        tf.compat.v1.add_to_collection('step', eval_model.ac)
        ...
        def train(obs, states, rewards, actions, values):
            ...
            if log_interval > 0 and (self.num_steps_trained % self.log_interval == 0):
                ...
                self.summary_writer.add_summary(tf.compat.v1.Summary.FromString(summary_str), global_step)

            self.num_steps_trained += 1

            return policy_loss, value_loss, policy_entropy, ap

        saver = tf.compat.v1.train.Saver(max_to_keep=keep_model)
        ...
        if log_interval > 0:
            self.summary_writer = tf.compat.v1.summary.FileWriter(logdir, graph_def=sess.graph_def)
        else:
...
if __name__ == '__main__':
    ...
    logdir = '/Users/mackj/OneDrive/Desktop/logs/LSTM_A2C-lala'
